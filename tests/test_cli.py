...
def test_cli_ingest(mock_ingest, tmp_path):
    ...
    assert "Stored in ./notes-qa.db" in result.output
...
def test_cli_ask(mock_retrieve, mock_generate):
    ...
    assert 'Token-bucket rate limiting smooths traffic [rate-limiting-notes.md, "Token Bucket"].' in result.output
    assert "Sources:" in result.output
    assert '  - rate-limiting-notes.md — "Token Bucket"' in result.output


@patch("notes_qa.cli.retrieve_chunks")
def test_cli_ask_empty_index(mock_retrieve):
    mock_retrieve.return_value = []
    runner = CliRunner()
    result = runner.invoke(main, ["ask", "What did I write?"])
    assert result.exit_code == 0
    assert "I could not find any notes in the index" in result.output
